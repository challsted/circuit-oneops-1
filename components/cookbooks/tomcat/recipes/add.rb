...
(node['tomcat'].key?('protocol') && !node['tomcat']['protocol'].empty?) ?
  node.set['tomcat']['connector']['protocol'] = node['tomcat']['protocol'] :
  node.set['tomcat']['connector']['protocol'] = 'HTTP/1.1'

(node['tomcat'].key?('advanced_connector_config') && !node['tomcat']['advanced_connector_config'].empty?) ?
...
template '/etc/logrotate.d/tomcat' do # ~FC033
...
# IGNORE Food Critic reports an issue of a missing template regardless of its presence
template '/opt/nagios/libexec/check_tomcat.rb' do # ~FC033
...
# IGNORE Food Critic reports an issue of a missing template regardless of its presence
template '/opt/nagios/libexec/check_ecv.rb' do # ~FC033
...
# IGNORE Food Critic reports an issue of a missing template regardless of its presence
template '/opt/nagios/libexec/check_tomcat_app_version.sh' do # ~FC033
  source 'check_tomcat_app_version.sh.erb'
  variables(versioncheckscript: node['versioncheckscript'])
  owner 'oneops'
  group 'oneops'
  mode '0755'
end
