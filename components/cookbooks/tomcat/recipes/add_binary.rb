...
if source_list.empty?
  ...
  if services.nil? || !services.key?(:mirror)
  ...
  if mirrors.nil? || !mirrors.key?('apache')
    Chef::Log.error("Please make sure  cloud '#{cloud_name}' has mirror service with 'apache' eg {apache=>http://archive.apache.org/dist}")
    exit 1
  end
...
# TODO: Research "Chef Guards"
unless node['tomcat']['post_startup_command'].nil? # ~FC023
...
if depends_on.nil? || depends_on.empty? || depends_on[0][:rfcAction] == 'delete'
  ...
  include_recipe 'tomcat::setwrapperattribs'
